...
export function Remi({ rid = "R0", mapData, speed = 0.002 }) {
  const group = useRef();
  ...
  useEffect(() => {
    cloned.traverse((obj) => {
      if (obj.isMesh && obj.material) {
        ...
        } else if (/top|shirt|pant|body|bottom/i.test(mat.name)) {
  ...
  // 🧭 Sélection aléatoire d’un itinéraire
  const pickRoute = () => {
    const nodes = mapData.nodes;
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // ✅ empêche le squelette de reculer
  ...
  }, [animations, mixer, cloned, clock, points3D, seg, speed, rid]);
  ...
  return (
    <group ref={group}>
      <primitive object={cloned} scale={randomScale/2} />
    </group>
  );
}
